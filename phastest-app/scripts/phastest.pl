...
my $diamond_database="swissprot.dmnd";
...
my $cluster_head_bac_database = "$database_dir/$diamond_database"; # Uses path on head node
my $cluster_child_bac_database = "$database_dir/$diamond_database"; # Uses path on each child node
...
# fasta seq or contigs file, so use prodigal or FragGeneScan to predict ORF and create .ppt .fna .faa files;
if ($flag eq '-s' or $flag eq '-c'){ 
...
if ($flag ne '-g' && !-e "$jobs_dir/$num/ncbi.out.non_hit_pro_region"){
	...
	system("echo 'Start DIAMOND on non-hit regions at $time sec' >> $log_file");
