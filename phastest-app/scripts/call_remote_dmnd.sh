...
progress=0
while [ -e /proc/$pid ]; do
    # Read last line of diamond log if available; suppress errors when file is temporarily missing
    most_recent_line=$(tail -n 1 "$diamond_log" 2>/dev/null || true)
    ...
    echo "$ref_block/$ref_block_total $shape/$shape_total $index_chunk/$index_chunk_total" >> test.txt
