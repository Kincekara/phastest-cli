...
echo "$fraggenescan_exec -genome=$fna_file_basename -out=$fna_file_basename -complete=0 -train=complete" >> $log_file
eval "$fraggenescan_exec -genome=$fna_file_basename -out=$fna_file_basename -complete=0 -train=complete" >/dev/null 2>&1 &
pid=$!
